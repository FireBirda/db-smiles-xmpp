...
import com.digitalbuana.smiles.awan.model.MUCConfigModel;
...
public class MUCManager implements OnLoadListener, OnPacketListener {
	...
	private void onLoaded(Collection<RoomChat> roomChats, Collection<RoomChat> needJoins) {
		for (RoomChat roomChat : roomChats) {
			...
			joinRoom(roomChat.getAccount(), roomChat.getRoom(), true);
		}
	...
	public void requestToWriteRoom(final String account, final String room,
	...
	public void joinRoom(final String account, final String room, boolean requested) {
		...
		try {
			...
			bm.addBookmarkedConference(account, room, true, account, "");
		...
		thread = new Thread("Join to room " + room + " from " + account) {
			...
			public void run() {
				...
				try {
					...
					/*CreateBookmarkHelper cbh = new CreateBookmarkHelper(account, room, true);
					cbh.setType(Type.SET);
					cbh.setFrom(account+"@"+AppConstants.XMPPServerHost+"/"+AppConstants.XMPPConfResource);
					xmppConnection.sendPacket(cbh);*/
	...
	public void setMucConfig(MultiUserChat multiUserChat){
		
		try {
			MUCConfigModel mcm = new MUCConfigModel(multiUserChat);	        
	        mcm.setAllowUserToInvite(false);	        
	        mcm.setIsMemberOnly(true);
	        mcm.setIsModeratedRoom(true);
	        mcm.setIsPersistentRoom(true);
	        mcm.setIsPublicRoom(false);	        
	        List<String> broadcastPresenceList = new ArrayList<String>();
	        broadcastPresenceList.add("moderator");
	        broadcastPresenceList.add("participant");
	        mcm.setPresenceBroadcast(broadcastPresenceList);	        
	        mcm.setRoomDescription(multiUserChat.getNickname());	        
	        mcm.setUserCanChangeSubject(false);	        
	        List<String> broadcastList = new ArrayList<String>();
	        broadcastList.add("moderators");
	        mcm.setVisibleJIDTo(broadcastList);	        
	        mcm.submitResult();
	    } catch (Exception e) {
	        //e.printStackTrace();
	    	//Log.e(TAG, e.getMessage());
	...
	public void leaveRoom(String account, String room) {
		...
		roomChat.newAction(roomChat.getNickname(), null, ChatAction.leave, "", false);
	...
	public void invite(String account, String room, String user)
		...
		roomChat.newAction(roomChat.getNickname(), user, ChatAction.invite_sent, "", false);
	}
