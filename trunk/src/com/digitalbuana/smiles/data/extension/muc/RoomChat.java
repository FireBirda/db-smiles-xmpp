...
import com.digitalbuana.smiles.xmpp.muc.Affiliation;
import com.digitalbuana.smiles.xmpp.muc.MUC;
import com.digitalbuana.smiles.xmpp.muc.Role;
...
public class RoomChat extends AbstractChat {
		...
		if (packet instanceof Message) {
			...
			if (subject != null) {
				...
				RosterManager.getInstance().onContactChanged(account,
						bareAddress);
				newAction(resource, subject, ChatAction.subject, "", false);
			} else {
	...
	private void onAvailable(String resource) {
		if (isSelf(resource)) {
			...
			if (isRequested()) {
				...
				if (showStatusChange())
					newAction(resource, null, ChatAction.complete, "", false);
			}
		} else {
			if (state == RoomState.available)
				if (showStatusChange())
					newAction(resource, null, ChatAction.join, "", false);
		}
	}
	...
	private void onLeave(String resource) {
		if (showStatusChange())
			newAction(resource, null, ChatAction.leave, "", false);
		if (isSelf(resource)) {
	...
	private void onKick(String resource, String actor) {
		if (showStatusChange())
			newAction(resource, actor, ChatAction.kick, "", false);
		if (isSelf(resource))
	...
	private void onBan(String resource, String actor) {
		if (showStatusChange())
			newAction(resource, actor, ChatAction.ban, "", false);
		if (isSelf(resource))
			MUCManager.getInstance().leaveRoom(account, user);
	}
	...
	private void onRename(String resource, String newNick) {
		if (showStatusChange())
			newAction(resource, newNick, ChatAction.nickname, "", false);
	}
