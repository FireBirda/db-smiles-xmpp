...
public class FriendsWaitingMeApproveManager {
	// Database fields
	...
	public FriendsWaitingMeApproveManager() {	
	...
	private void addFriend(String name, String jid){
		...
		long insertId = database.insert(MySQLiteHelper.TABLE_FRIENDSLIST_REQUEST, null, values);
		Cursor cursor = database.query(MySQLiteHelper.TABLE_FRIENDSLIST_REQUEST,  allFriends, MySQLiteHelper.COLUMN_ID + " = " + insertId, null, null, null, null);
		cursor.moveToFirst();
	...
	public void removeFriends(FriendsModel friends) {
		...
		for(int x=0;x<list.size();x++){
			if(list.get(x).equals(friends)){
				 ...
				 database.delete(MySQLiteHelper.TABLE_FRIENDSLIST_REQUEST, MySQLiteHelper.COLUMN_ID+ " = " + id, null);
			}
		}
	}
	...
	public ArrayList<FriendsModel> getAllFriends() {
		...
		Cursor cursor = database.query(MySQLiteHelper.TABLE_FRIENDSLIST_REQUEST, allFriends, null, null, null, null, null);
	    cursor.moveToFirst();
