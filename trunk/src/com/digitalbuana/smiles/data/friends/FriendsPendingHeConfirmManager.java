...
public class FriendsPendingHeConfirmManager {
	// Database fields
	...
	public FriendsPendingHeConfirmManager() {	
	...
	private void addFriend(String name, String jid){
		...
		long insertId = database.insert(MySQLiteHelper.TABLE_FRIENDSLIST_PENDING, null, values);
		Cursor cursor = database.query(MySQLiteHelper.TABLE_FRIENDSLIST_PENDING,  allFriends, MySQLiteHelper.COLUMN_ID + " = " + insertId, null, null, null, null);
		cursor.moveToFirst();
	...
	public void removeFriends(FriendsModel friends) {
		...
		for(int x=0;x<list.size();x++){
			if(list.get(x).equals(friends)){
				 ...
				 database.delete(MySQLiteHelper.TABLE_FRIENDSLIST_PENDING, MySQLiteHelper.COLUMN_ID+ " = " + id, null);
			}
		}
	}
	...
	public ArrayList<FriendsModel> getAllFriends() {
		...
		Cursor cursor = database.query(MySQLiteHelper.TABLE_FRIENDSLIST_PENDING, allFriends, null, null, null, null, null);
	    cursor.moveToFirst();
