...
public class OccupantListAdapter extends BaseAdapter implements
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		final View view;
		if (convertView == null) {
			view = activity.getLayoutInflater().inflate(R.layout.occupant_list_item, parent, false);
		} else {
			view = convertView;
		}
		...
		final ImageView statusModeView = (ImageView) view.findViewById(R.id.status_mode);
		
		if (MUCManager.getInstance().getNickname(account, room).equalsIgnoreCase(occupant.getNickname()))
			avatarView.setImageDrawable(AvatarManager.getInstance().getAccountAvatar(account));
		else
			avatarView.setImageDrawable(AvatarManager.getInstance().getOccupantAvatar(room + "/" + occupant.getNickname()));
		
		affilationView.setImageLevel(occupant.getAffiliation().ordinal());
		nameView.setText(StringUtils.replaceStringEquals(occupant.getNickname()));
		int textStyle;
...
//		nameView.setTextAppearance(activity, textStyle);
		statusTextView.setText(StringUtils.replaceStringEquals(occupant.getStatusText()));
