 ...
 */
package com.digitalbuana.smiles.ui.widget;
...
import com.digitalbuana.smiles.ui.adapter.SaveStateAdapter;
...
public class PageSwitcher extends ViewGroup implements AnimationListener {
	...
	private final Runnable pagesHideRunnable = new Runnable() {
		...
		public void run() {
			...
			if (selectedView != null)
				selectedView.findViewById(R.id.chat_page).startAnimation(pagesHideAnimation);
			if (visibleView != null) {
				visibleView.findViewById(R.id.chat_page).setVisibility(View.GONE);
	...
	private View getView(int position, int x, View convertView, boolean update,
		...
		if (update || layout || view.getLeft() != x) {
			...
			view.layout(x, 0, x + view.getMeasuredWidth(),view.getMeasuredHeight());
		}
		return view;
	}
