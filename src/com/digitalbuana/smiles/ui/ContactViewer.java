...
import com.digitalbuana.smiles.data.extension.vcard.OnVCardListener;
...
import com.digitalbuana.smiles.ui.helper.ManagedPreferenceActivity;
import com.digitalbuana.smiles.ui.widget.StatusPreference;
import com.digitalbuana.smiles.xmpp.address.Jid;
import com.digitalbuana.smiles.xmpp.vcard.Address;
import com.digitalbuana.smiles.xmpp.vcard.AddressProperty;
import com.digitalbuana.smiles.xmpp.vcard.AddressType;
import com.digitalbuana.smiles.xmpp.vcard.Email;
import com.digitalbuana.smiles.xmpp.vcard.EmailType;
import com.digitalbuana.smiles.xmpp.vcard.NameProperty;
import com.digitalbuana.smiles.xmpp.vcard.Organization;
import com.digitalbuana.smiles.xmpp.vcard.Telephone;
import com.digitalbuana.smiles.xmpp.vcard.TelephoneType;
import com.digitalbuana.smiles.xmpp.vcard.VCard;
import com.digitalbuana.smiles.xmpp.vcard.VCardProperty;
import com.digitalbuana.smiles.xmpp.vcard.VCardProvider;
...
public class ContactViewer extends ManagedPreferenceActivity implements
	...
	private static final String SAVED_VCARD = "com.digitalbuana.smiles.ui.ContactViewer.SAVED_VCARD";
	private static final String SAVED_VCARD_ERROR = "com.digitalbuana.smiles.ui.ContactViewer.SAVED_VCARD_ERROR";
	...
	private void updateContact() {
		...
		if (rosterContact != null && rosterContact.isConnected())
			for (ResourceItem resourceItem : PresenceManager.getInstance() .getResourceItems(account, bareAddress)) {
				...
				if (clientInfo == null) {
					...
					client = getString(R.string.contact_viewer_client_info, name, type);
				}
	...
	private void updateVCard() {
		if (vCard == null)
			return;
		setValue(R.string.vcard_nick_name,vCard.getField(VCardProperty.NICKNAME));
		setValue(R.string.vcard_formatted_name, vCard.getFormattedName());
		setValue(R.string.vcard_prefix_name,vCard.getField(NameProperty.PREFIX));
		setValue(R.string.vcard_given_name, vCard.getField(NameProperty.GIVEN));
		setValue(R.string.vcard_middle_name, vCard.getField(NameProperty.MIDDLE));
		setValue(R.string.vcard_family_name, vCard.getField(NameProperty.FAMILY));
		setValue(R.string.vcard_suffix_name, vCard.getField(NameProperty.SUFFIX));
		...
		for (Address address : vCard.getAddresses()) {
			...
			PreferenceScreen addressScreen = createTypedCategory(R.string.vcard_address, types, value);
			for (AddressProperty property : AddressProperty.values())
				addPreferenceScreen(addressScreen, ADDRESS_PROPERTY_MAP.get(property), address.getProperties().get(property));
		}
	...
	private PreferenceScreen createTypedCategory(int title, String types, String value) {
	...
	private PreferenceScreen addPreferenceScreen(PreferenceGroup container, int title, String summary) {
	...
	public static Intent createIntent(Context context, String account, String user) {
		return new EntityIntentBuilder(context, ContactViewer.class).setAccount(account).setUser(user).build();
	}
