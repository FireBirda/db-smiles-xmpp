...
import com.digitalbuana.smiles.data.OnUnloadListener;
...
import com.digitalbuana.smiles.data.account.OnAccountSyncableChangedListener;
...
import com.digitalbuana.smiles.utils.DummyCursor;
...
public class SyncManager implements OnLoadListener, OnUnloadListener,
	...
	private Entry<Long, String> getFirstEntry(HashMap<Long, String> map,
		...
		for (Entry<Long, String> entry : map.entrySet())
			if (result == null)
				...
				//LogManager.w(this, "Remove data: " + entry.getKey() + ": "+ entry.getValue());
	...
	private void onLoaded(Collection<RosterContact> rosterContacts, Collection<RosterGroup> rosterGroups) {
		createAccounts = true;
		accountManager.addOnAccountsUpdatedListener(this, null, true);
		registeredOnAccountsUpdatedListener = true;

			HashSet<String> enabledAccounts = new HashSet<String>();
			for (String account : com.digitalbuana.smiles.data.account.AccountManager.getInstance().getAllAccounts()) {
				AccountItem accountItem = com.digitalbuana.smiles.data.account.AccountManager.getInstance().getAccount(account);
			...
			for (RosterContact rosterContact : rosterContacts)
				if (syncableAccounts.contains(rosterContact.getAccount())) {
					rosterContact.setEnabled(enabledAccounts.contains(rosterContact.getAccount()));
...
//					LogManager.w(this, "Disable synchronization for: "+ account);
//					if (com.digitalbuana.smiles.data.account.AccountManager.getInstance().getAccount(account) != null)
//						com.digitalbuana.smiles.data.account.AccountManager.getInstance().setSyncable(account, false);
				//}
			}
		});
	}
